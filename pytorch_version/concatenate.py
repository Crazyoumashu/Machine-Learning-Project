...
parser.add_argument('--train_url', type=str, default='s3://test-pytorch/fer-2013-data/model/V0006/',
...
criterion = nn.CrossEntropyLoss()
optimizer = optim.SGD(netClassifier.parameters(), lr=opt.lr, momentum=0.9, weight_decay=5e-4)
temp_train_loss = []
temp_train_acc = []
temp_putest_loss = []
temp_putest_acc = []
...
def layers(x, nett):
    out = F.relu(nett.bn1(nett.conv1(x)))
    out = nett.layer1(out)
    out = nett.layer2(out)
    out = nett.layer3(out)
    out = nett.layer4(out)
    ...
    out = F.dropout(out, p=0.5, training=nett.training)
    return out
...
def PublicTest(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(PublicTestloader):
        ...
        conTensor1 = net1.features(inputs).view(bs * ncrops, -1)
        conTensor2 = layers(inputs, net2).view(bs * ncrops, -1)
        if use_cuda:
            conTensor1, conTensor2 = conTensor1.cuda(), conTensor2.cuda()
        resTensor = torch.cat((conTensor1, conTensor2), 1)
        #print(resTensor.shape)
   ...
   # Save checkpoint.
    PublicTest_acc = float(100.*correct)/float(total)
    print(PublicTest_acc)
    #if((epoch+1)%10==0):
    ...
    if PublicTest_acc > best_PublicTest_acc:
        ...
        state = {
            'net': netClassifier.state_dict() if use_cuda else net,
            'acc': PublicTest_acc,
            'epoch': epoch,
        }
...
def PrivateTest(epoch):
    ...
    print("Private Acc:", float(100. * correct) / float(total))
    # Save checkpoint.
    PrivateTest_acc = float(100. * correct) / float(total)
    print(PrivateTest_acc)
    #if ((epoch + 1) % 10 == 0):
...
net.load_state_dict(torch.load('./FER2013_VGG19/PublicTest_ad251_model.t7', map_location='cpu')['net'])
net1 = VGG('VGG19')
net1.load_state_dict(torch.load('./FER2013_VGG19/PublicTest_ad251_model.t7', map_location='cpu')['net'])
if use_cuda:
    net1.cuda()
net2 = ResNet18()
net2.load_state_dict(torch.load('./FER2013_Resnet18/PrivateTest_model.t7', map_location='cpu')['net'])
if use_cuda:
    net2.cuda()
print("==> Just DO ITT!")
